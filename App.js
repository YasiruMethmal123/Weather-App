...
fetch(
  "https://api.weatherapi.com/v1/current.json?q=sri-lanka&key=3c5b19c5255e485db7a42702240909 "
)
...
document.getElementById("forcastgrid").innerHTML = code_hourly_forcast;
...
for (let i = 0; i < data.length; i++) {
  code_weather_details += `
    <div class="col fw-medium">
        <div class=" p-2 rounded-2 py-3">
            <p style="color:white; font-size: 15px;">${data[i][0]}</p>
            <h6 class="fs-25">${data[i][1]}</h6>
        </div>
    </div>`;
}

document.getElementById("weatherdetailgrid").innerHTML =
  code_weather_details;
}